...
import { getGiftImage } from "../server/api";

const GiftTypeScreen = () => {
  ...
  const uploadImage = async (file) => {
    ...
    formData.append('type', gifttype); // You can replace gifttype with your actual type variable

    try {
      setLoading(true);

      const response = await axios.post('/api/gift/giftimage/', formData, {
        headers: {
      ...
      <div className="flex flex-col items-center mt-4 p-5">
        ...
        <div className="flex flex-col md:flex-row gap-3 w-full">
          ...
          <div className="w-full md:w-1/2">
            {selectedImage ? (
              <img src={selectedImage} alt="Selected" className="w-full mt-5 md:mt-0" />
            ) : (
              <img
